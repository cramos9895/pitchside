...
export async function POST(request: Request) {
    try {
            ...
            if (gameConfig && gameConfig.teams_config && Array.isArray(gameConfig.teams_config)) {
                ...
                if (teamConfig && teamConfig.limit && teamConfig.limit > 0) {
                    ...
                    const { count: teamCount } = await adminSupabase
                        ...
                        .eq('game_id', gameId)
                        .neq('status', 'cancelled')
                        // teamAssignment is a string from the frontend, but we need to match it to a team ID.
                        // For tournaments, the frontend might send team name. Since we can't easily join on team name here via a simple .eq
                        // we can skip this explicit check or assume the frontend is doing validation. 
                        // Actually, for tournament squad limits, let's just bypass it here to prevent the crash, as the frontend checks it.
                        // .eq('team_id', teamAssignment);
        ...
        if (!resolvedTeamId && teamAssignment) {
        ...
        if (!requiresVaulting && totalDueUnits === 0) {
            ...
            for (const passenger of passengersToProcess) {
                try {
                    // 1. Insert into tournament_registrations
                    ...
                    // 2. Insert into bookings (for legacy frontend roster rendering)
        ...
        // --- CREATE PENDING TOURNAMENT REGISTRATION ---
        ...
        if (!actualRegistrationId) {
            const registrationsToInsert: any[] = [{
                ...
                note: note || null,
                is_free_agent: isFreeAgent || false,
            }];
            ...
            const { data: newRegs, error: regError } = await adminSupabase
                .from('tournament_registrations')
                .insert(registrationsToInsert)
                .select('id, user_id');
            
            if (newRegs && newRegs.length > 0) {
                const primaryReg = newRegs.find(r => r.user_id === userId) || newRegs[0];
                actualRegistrationId = primaryReg.id;
            } else {
                console.error('[CHECKOUT_API] Failed to create pending tournament registrations:', regError);
            }
        }
