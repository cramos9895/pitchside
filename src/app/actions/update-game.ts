...
export async function updateGame(gameId: string, formData: any) {
    ...
    const { error } = await supabase
        ...
        .update({
            ...
            is_league: (formData.event_type === 'tournament' || formData.event_type === 'league' || formData.league_format === 'rolling'),
            league_format: formData.league_format,
            ...
            roster_freeze_date: formData.roster_freeze_date,
            regular_season_start: formData.regular_season_start,
            playoff_start_date: formData.playoff_start_date,
            strict_waiver_required: formData.strict_waiver_required,
            waiver_details: formData.waiver_details,
            ...
            payment_collection_type: formData.payment_collection_type,
            cash_fee_structure: formData.cash_fee_structure,
            cash_amount: formData.cash_amount,
            allow_free_agents: formData.allow_free_agents,
            charge_team_registration_fee: formData.charge_team_registration_fee,
            league_end_date: formData.league_end_date,
            team_signup_cutoff: formData.team_signup_cutoff,
            team_registration_fee: formData.team_registration_fee,
            deduct_team_reg_fee: formData.deduct_team_reg_fee,
            player_registration_fee: formData.player_registration_fee,
        })
        .eq('id', gameId);
