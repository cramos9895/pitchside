...
import { ArrowRight, AlertTriangle, CheckCircle2, Trophy, CreditCard, ScrollText } from 'lucide-react';
import { cn } from '@/lib/utils';
import { registerCaptain, registerFreeAgent } from '@/app/actions/league-registration';
import { EmbeddedCheckoutModal } from '@/components/EmbeddedCheckoutModal';

export function LeagueRegistrationClient({ 
    leagueId, 
    leagueName, 
    teamPrice, 
    faPrice,
    ...
    const finalizeTeamRegistration = async (payload: any) => {
        ...
        try {
            ...
            const res = await registerCaptain(formData);
            if (res.success && res.teamId) {
                if (isRolling) {
                    router.push(`/rolling-leagues/${leagueId}`);
                } else {
    ...
    const handleTeamSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!payload.liability_acknowledged && !isCashLeague) {
            ...
            if (depositAmount > 0 && !isCashLeague) {
                ...
                pendingFormData.append('leagueId', leagueId);
                pendingFormData.append('status', 'pending');
                
                const res = await registerCaptain(pendingFormData);
                if (res.success && res.registrationId) {
                    setCurrentRegistrationId(res.registrationId);
                    setCurrentTeamId(res.teamId || null);
                    ...
                    const checkoutRes = await fetch('/api/checkout', {
                        ...
                        body: JSON.stringify({
                            ...
                            userId: userId,
                            price: depositAmount,
                            title: `Team Deposit: ${leagueName}`,
                            registrationId: res.registrationId,
                            teamId: res.teamId,
                            isLeagueCaptainVaulting: true,
                            type: 'league'
                        })
                    });
    ...
    const finalizeFARegistration = async (payload: any) => {
        ...
        try {
            ...
            const res = await registerFreeAgent(formData);
            if (res.success) {
                if (isRolling) {
                    router.push(`/rolling-leagues/${leagueId}`);
                } else {
    ...
    const handleFASubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (faTotalOnlinePrice > 0) {
                ...
                pendingFormData.append('leagueId', leagueId);
                pendingFormData.append('status', 'pending');
                pendingFormData.append('positions', JSON.stringify(payload.positions));
                
                const res = await registerFreeAgent(pendingFormData);
                if (res.success && res.registrationId) {
                    setCurrentRegistrationId(res.registrationId);
                    setCurrentEventType(res.eventType || 'league');
        ...
        <div className="space-y-6 pt-6 border-t border-white/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {description && (
                    <div className="space-y-3">
                        <div className="text-[10px] font-black uppercase text-pitch-secondary tracking-widest">League Rules</div>
            ...
            {strictWaiverRequired && (
                <label className="flex items-center gap-3 cursor-pointer group pt-2">
                    ...
                    />
                    <span className="text-xs font-bold text-gray-400 group-hover:text-white uppercase tracking-widest select-none">I have read and agree to the league terms.</span>
                </label>
            )}
        </div>
    );
