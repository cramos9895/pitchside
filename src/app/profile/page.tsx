...
import { Game, Booking, ProfileWithUI, Match, Team } from "@/types/index";

import type { User as SupabaseUser } from '@supabase/supabase-js';

interface BookingWithGame extends Booking {
    game?: Game & {
        matches?: Match[];
        bookings?: { is_winner: boolean | null }[];
    };
...
export default function ProfilePage() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            // Calculate Stats
                                                const caps = bookingsData?.filter((b: BookingWithGame) => b.status === 'paid').length || 0;
            ...
            const validBookings = bookingsData?.filter((b: BookingWithGame) => {
                                                                const g = b.game;
                // Count played games
                ...
                // Pre-compute if the game had any winner recorded to distinguish Losses from True Draws (specifically for legacy games)
                                                                const gameHadWinner = g.bookings?.some((bk: { is_winner: boolean | null }) => bk.is_winner === true) || !!g.winning_team_assignment;
        ...
        <div className="min-h-screen bg-pitch-black text-white p-6 pt-32 font-sans flex flex-col items-center">
            ...
            <div className="w-full max-w-2xl px-4 pb-20">
                ...
                <div className="space-y-4">
                                                                                {bookings.map((booking: BookingWithGame) => {
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className={cn(
                                        ...
                                        {result === 'draw' ? 'Draw' : result === 'win' ? 'Win' : 'Loss'}
                                                                                ...
                                                                                {bookings.filter((b: BookingWithGame) => b.calculated_result).length === 0 && (
