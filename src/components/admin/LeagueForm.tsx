...
export function LeagueForm({ initialData, action = 'create', onSuccess }: LeagueFormProps) {
    ...
    const handleLoadTemplate = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (data.regular_season_start) setRegularSeasonStart(new Date(data.regular_season_start).toISOString().slice(0, 16));
        ...
        setAllowFreeAgents(data.allow_free_agents ?? true);
        ...
        setMinGamesGuaranteed(data.min_games_guaranteed ?? 8);
    ...
    const handleSaveTemplate = async () => {
        ...
        try {
            ...
            const res = await saveTemplate(templateName, 'league', payload);
